...
out = log_dir / 'db-ping-local.out.log'
err = log_dir / 'db-ping-local.err.log'
mvn = r'D:\apache-maven-3.9.15-bin\apache-maven-3.9.15\bin\mvn.cmd'
proc = subprocess.Popen([mvn, 'spring-boot:run', '-Dspring-boot.run.profiles=local', '-Dspring-boot.run.arguments=--server.port=18080'], cwd=str(root), stdout=out.open('w', encoding='utf-8'), stderr=err.open('w', encoding='utf-8'))
try:
    health = None
    for _ in range(60):
        time.sleep(1)
