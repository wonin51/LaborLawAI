...
out = log_dir / 'db-ping-local-18082.out.log'
err = log_dir / 'db-ping-local-18082.err.log'
mvn = r'D:\apache-maven-3.9.15-bin\apache-maven-3.9.15\bin\mvn.cmd'
proc = subprocess.Popen([mvn, 'spring-boot:run', '-Dspring-boot.run.profiles=local', '-Dspring-boot.run.arguments=--server.port=18082'], cwd=str(root), stdout=out.open('w', encoding='utf-8'), stderr=err.open('w', encoding='utf-8'))
try:
    health = None
    for _ in range(60):
        time.sleep(1)
        if proc.poll() is not None:
            break
        try:
            with urllib.request.urlopen('http://127.0.0.1:18082/api/health', timeout=2) as r:
    ...
    print('HEALTH=', health)
    try:
        with urllib.request.urlopen('http://127.0.0.1:18082/api/db/ping', timeout=10) as r:
