...
out = log_dir / 'db-ping-final.out.log'
err = log_dir / 'db-ping-final.err.log'
env = os.environ.copy()
env['DB_PASSWORD'] = '12181168'
env['OPENAI_API_KEY'] = 'dummy-key-for-db-ping-test'
mvn = r'D:\apache-maven-3.9.15-bin\apache-maven-3.9.15\bin\mvn.cmd'
proc = subprocess.Popen([mvn, 'spring-boot:run', '-Dspring-boot.run.arguments=--server.port=18080'], cwd=str(root), env=env, stdout=out.open('w', encoding='utf-8'), stderr=err.open('w', encoding='utf-8'))
try:
    health = None
    for _ in range(60):
        time.sleep(1)
